...
You can also use the `.query()` method for HEAD requests. The following will produce the path `/appUsers?email=joe@smith.com`.
...
In the _Node_ client Basic auth can be in the URL as "appUser:pass":
...
Much like form fields in HTML, you can set field values with `.field(name, value)` and `.field({name: value})`. Suppose you want to upload a few images with your name and email, your request might look something like this:
     ...
     request
       .post('/upload')
       .field('appUser[name]', 'Tobi')
       .field('appUser[email]', 'tobi@learnboost.com')
