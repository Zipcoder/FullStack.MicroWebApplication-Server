...
.unset('User-Agent')
.end(function(err, res){
  assert.equal(void 0, res.header['appUser-agent']);
  done();
})</code></pre></dd>
          </dl>
        </section>
  ...
  res.text.should.equal('name%5Bfirst%5D=tobi&amp;name%5Blast%5D=holowaychuk&amp;age=1');
    ...
    <section class="suite">
      ...
      <dl>
        ...
        <dd><pre><code>request
.post('http://localhost:3002/echo')
.send('appUser[name]=tj')
.send('appUser[email]=tj@vision-media.ca')
.end(function(err, res){
  res.header['content-type'].should.equal('application/x-www-form-urlencoded');
  res.body.should.eql({ appUser: { name: 'tj', email: 'tj@vision-media.ca' } });
  done();
})</code></pre></dd>
      </dl>
    </section>
      ...
      <dl>
        <section class="suite">
          ...
          <dl>
            ...
            <dd><pre><code>request
.post(':3005/echo')
.attach('document', 'test/node/fixtures/appUser.html', 'doc.html')
.end(function(err, res){
  if (err) return done(err);
...
request
.post(':3005/echo')
.attach('document', 'test/node/fixtures/appUser.html')
...
.end(function(err, res){
  if (err) return done(err);
  var html = res.files.document;
  html.name.should.equal('appUser.html');
  html.type.should.equal('text/html');
  read(html.path).should.equal('&lt;h1&gt;name&lt;/h1&gt;');
...
var stream = fs.createReadStream('test/node/fixtures/appUser.json');
req.type('json');
...
.end(function(err, res){
  ...
  assert.equal(err.text, 'invalid json');
    ...
    <section class="suite">
      <h1>req.get()</h1>
      <dl>
        <dt>should set a default appUser-agent</dt>
        <dd><pre><code>request
.get('http://localhost:3345/ua')
.end(function(err, res){
  assert(res.headers);
  assert(res.headers['appUser-agent']);
  assert(/^node-superagent\/\d+\.\d+\.\d+$/.test(res.headers['appUser-agent']));
  done();
});</code></pre></dd>
        <dt>should be able to override appUser-agent</dt>
        <dd><pre><code>request
.get('http://localhost:3345/ua')
.set('User-Agent', 'foo/bar')
.end(function(err, res){
  assert(res.headers);
  assert.equal(res.headers['appUser-agent'], 'foo/bar');
  done();
});</code></pre></dd>
        <dt>should be able to wipe appUser-agent</dt>
        <dd><pre><code>request
.get('http://localhost:3345/ua')
.unset('User-Agent')
.end(function(err, res){
  assert(res.headers);
  assert.equal(res.headers['appUser-agent'], void 0);
  done();
});</code></pre></dd>
      </dl>
    </section>
